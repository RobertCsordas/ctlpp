...
MultilayerLSTMState = List[Tuple[torch.Tensor, torch.Tensor]]

def add_eos(input: torch.Tensor, lengths: torch.Tensor, eos_id: int, batch_dim: int = 1, pad_val=0):
...
class DecoderOutput:
    ...
    def last_token(self) -> torch.Tensor:
        return self.data.argmax(-1)

    def remove_front(self, n: int):
        return DecoderOutput(self.data[n:])

    @classmethod
    def merge(cls, others: List):
        return cls(torch.cat([i.data for i in others], 0))
...
class Encoder(torch.nn.Module):
    ...
    def construct(self, hidden_size: int, n_layers: int, embedding_size: int, lstm: torch.nn.Module):
        self.embedding = torch.nn.Embedding(self.vocabulary_size + 1, embedding_size)
    ...
    def run(self, input: torch.Tensor, lengths: torch.Tensor) -> Tuple[torch.Tensor, MultilayerLSTMState]:
...
class BidirectionalLSTMEncoder(Encoder):
    def __init__(self, vocabulary_size: int, hidden_size: int, n_layers: int, embedding_size: int, dropout: float, lstm: torch.nn.Module):
