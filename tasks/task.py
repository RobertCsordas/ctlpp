...
class Task:
    ...
    def set_lr(self):
        has_builtin_warmup = {"noam"}

        if isinstance(self.lr_scheduler, torch.optim.lr_scheduler._LRScheduler):
            self.lr_scheduler.step(self.helper.state.iter)
            if self.helper.state.iter % 100 == 0:
                self.helper.log({"lr": self.lr_scheduler.get_last_lr()[0]})
        else:
            self.set_optimizer_lr(self.lr_scheduler.get(self.helper.state.iter))

        if self.helper.args.lr_warmup > self.helper.state.iter and \
                self.helper.args.lr_sched.type not in has_builtin_warmup:
    ...
    def validate_on(self, set: torch.utils.data.Dataset, loader: torch.utils.data.DataLoader) -> Tuple[Any, float]:
        ...
        with torch.no_grad():
            ...
            for i, d in enumerate(tqdm(loader)):
                ...
                if self.helper.dist_env.is_distributed:
                    ...
                    torch.distributed.all_gather_object(alist, (digits, d, this_loss))

                    for digits, d, this_loss in alist:
                        if digits is None:
                            continue
                        loss_sum += this_loss
                        test.step(digits, d)
        ...
        if (not bucketed) and (self.helper.dist_env.is_work_uneven(l)) and (not self.helper.dist_env.has_extra_work(l)):
            alist = [None] * self.helper.dist_env.world_size
            # if the size of the dataset is not divisible by the world size, some workers have an extra work item. If
            # we don't have one, send Nones, so they won't hang
            torch.distributed.all_gather_object(alist, (None, None, None))
