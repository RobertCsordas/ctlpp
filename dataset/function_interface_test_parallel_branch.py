...
class FunctionInterfaceTestParallelBranch(FunctionInterfaceTestBase):
    VERSION = 4

    def config_id(self) -> str:
        return f"{self.N}_{self.depth[0]}_{self.depth[1]}_{self.seed}_{self.n_symbols}_{self.n_functions}_{self.reversed}_{self.overlap}_{self.symbol_overlap}"
    ...
    def init_data_tables(self, seed: np.random.RandomState):
        ...
        self.train_plan = self.create_plan({
            ...
            "restricted_edges": self.generate_restricted_overlaps(["a", "b"], "o", seed)
        }, self.depth)
...
class FunctionInterfaceTestParallelBranchClassification(FunctionInterfaceTestClassificationMixin, FunctionInterfaceTestParallelBranch):
    pass
